...
if [ "$old_image_id" != "$new_image_id" ]; then \
    ...
    rm -rf ./requirements/front_proxy/aegis/ModSecurity/; \
    docker rm -f modsec || true; \
    docker run -d --name modsec modsec; \
    docker cp modsec:/ModSecurity ./requirements/front_proxy/aegis/ModSecurity; \
    ...
    if [ ! -d "./requirements/front_proxy/aegis/ModSecurity" ]; then \
